import type {
  ExtensionAPI, ToolCallEvent, ToolCallReturn, ExtensionContext,
} from './types';
...
function block(env: HookEnvelope, fallbackReason: string): ToolCallReturn {
  return { block: true, reason: env.permissionDecisionReason ?? fallbackReason };
}

async function resolveAsk(
  env: HookEnvelope, ctx: ExtensionContext, fallbackTitle: string,
): Promise<ToolCallReturn | void> {
...
export function register(api: ExtensionAPI): void {
  api.on('tool_call', async (
    ...
    if (SECRET_GUARDED_TOOLS.has(evt.toolName)) {
      ...
      if (env?.permissionDecision === 'deny') return block(env, 'blocked by protect-secrets');
    ...
    if (evt.toolName === 'Bash') {
      ...
      if (dangerEnv?.permissionDecision === 'deny') return block(dangerEnv, 'blocked by block-dangerous-commands');
      ...
      if (compressEnv?.permissionDecision === 'deny') return block(compressEnv, 'blocked by bash-compress-hook');
